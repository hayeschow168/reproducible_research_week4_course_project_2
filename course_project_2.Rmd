...
```{r download, message = FALSE, cache.extra = tools::md5sum("repdata_data_StormData.csv.bz2")}
library(curl)
...
```{r calc_prop_damage, message = FALSE}
...
```{r prepare_plot_data, message = FALSE}
...
1. Across the United States, which types of events (as indicated in the <span style="color: red;">EVTYPE</span> variable) are most harmful with respect to population health?
  ```{r plot_prop_damaage, fig.align='center'}
  library(ggplot2)
...
2. Across the United States, which types of events have the greatest economic consequences?
  ```{r plot_fatalities, fig.align='center'}
  # plot fatalities
